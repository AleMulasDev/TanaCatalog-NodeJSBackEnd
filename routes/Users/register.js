...
/*
CREATE EVENT `delete_{$table}_{$column}_$value` 
...
router.post("/", function(req, res, next) {
  ...
  passRegex.test(req.body.password)){
    bcrypt.hash(req.body.password, 10, (err, password) => {
      if(!err){
        SQL.register(req.body.email, password, req.body.firstname, req.body.lastname)
        .then(()=>{

          //If registered, get user id
          SQL.getUserId(req.body.email).then(userID => {
            let now = moment();
