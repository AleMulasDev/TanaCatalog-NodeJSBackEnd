...
router.put('/', async function(req, res, next) {
  ...
  if(!requestError){
    ...
    try{
      ...
      if(canAccess){
        let alredyExist = await SQL.canAccessSection(req.body.sectionID, req.body.userID);
        if(alredyExist){
          let canModifyPermission = await SQL.canModifySectionPermission(req.body.sectionID, user.id);
          if(canModifyPermission){
            if(req.body.permission){
              let success = await SQL.updateUserPermission(req.body.userID, req.body.sectionID, req.body.permission);
              if(success){
                res.json({
                  status: 'ok'
                })
                utils.logInteraction('SectionUser', `${user.email} modified a user permission (ID: ${req.body.userID}) of section ${req.body.sectionID}`)
                return;
              }else{
                res.json({
    ...
    try{
      ...
      if(canAddAndModify && req.body.permission){
        let success = await SQL.addUserInSection(req.body.userID, req.body.sectionID, req.body.permission);
        if(success){
          res.json({
            status: 'ok'
          })
          utils.logInteraction('SectionUser', `${user.email} added a user (ID: ${req.body.userID}) to section ${req.body.sectionID}`)
        }else{
          res.json({
        ...
        if(canOnlyAdd){
          let success = await SQL.addUserInSectionWithoutPermission(req.body.userID, req.body.sectionID);
        if(success){
          res.json({
            status: 'ok'
          })
...
router.delete('/', async function(req, res, next) {
  ...
  if(!requestError){
    ...
    try{
      let canAccess = await SQL.canAccessSection(req.query.sectionID, user.id);
      if(canAccess){
        let userExist = await SQL.canAccessSection(req.query.sectionID, req.query.userID);
        if(userExist){
          let isOwner = await SQL.isSectionOwner(user.id, req.query.sectionID);
          if(isOwner){
            let success = await SQL.removeUserInSection(req.query.userID, req.query.sectionID);
            if(success){
              res.json({
                status: 'ok'
              })
              utils.logInteraction('SectionUser', `${user.email} removed a user (ID: ${req.query.userID}) to section ${req.query.sectionID}`)
            }else{
              res.json({
